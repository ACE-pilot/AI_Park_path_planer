...
class ParkEnv(gym.Env):
    ...
    def __init__(self, num_agents=3, render_mode=False): 
        ...
        self.action_spaces = {f'agent_{i}': self.single_action_space for i in range(self.num_agents)}

        # 观察空间：每个智能体都有相同的观察空间
        self.single_observation_space = spaces.Box(low=0, high=255, shape=self.observation_shape, dtype=np.uint8)
        self.observation_spaces = {f'agent_{i}': self.single_observation_space for i in range(self.num_agents)}
    ...
    def reset(self):
        ...
        self.start_positions = []
        self.end_positions = []
        min_distance = self.map_size * 0.5  # 最小间距

        for _ in range(self.num_agents):
            # 为每个智能体生成相同的起点和终点
            start_pos = self._get_point_on_boundary().astype(np.float32)
            end_pos = self._get_point_on_boundary().astype(np.float32)
            # 确保起点和终点有足够的距离
            while np.linalg.norm(start_pos - end_pos) < min_distance:
                end_pos = self._get_point_on_boundary().astype(np.float32)
            self.start_positions.append(start_pos)
            self.end_positions.append(end_pos)
            self.agent_positions.append(start_pos.copy())
    ...
    def _get_observations(self):
        # 获取每个智能体的观测，返回一个字典
        obs = {f'agent_{i}': self.map.copy() for i in range(self.num_agents)}
        return obs
    ...
    def step(self, actions):
        ...
        for i in range(self.num_agents):
            ...
            reward = self._calculate_reward(
                old_pos, new_pos, self.trajectories[i], i)
            rewards[agent_id] = reward

            # 判断是否到达终点或达到最大步数
            done = self._is_done(new_pos, self.end_positions[i]) or self.steps >= self.max_steps
            dones[agent_id] = done
    ...
    def _is_done(self, position, end_pos):
        # 判断是否到达终点附近
        if np.linalg.norm(position - end_pos) < 5:
            return True
        else:
            return False
    ...
    def render(self, mode='human'):
        ...
        # 绘制起点和终点
        for i in range(self.num_agents):
            plt.scatter(self.start_positions[i][0], self.start_positions[i][1],
                        marker='*', s=200, label=f'Agent {i+1} Start')
            plt.scatter(self.end_positions[i][0], self.end_positions[i][1],
                        marker='X', s=200, label=f'Agent {i+1} End')
