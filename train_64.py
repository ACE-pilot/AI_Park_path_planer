...
from cnn_model_64 import MAModel
...
from MADDPGWrapper_64 import MADDPGWrapper
import paddle
...
BATCH_SIZE = 1024  # For CNN-based MADDPG implementations, 64 or 128 is recommended.
...
def run_episode(env, agents):
    ...
    while not done and steps < MAX_STEP_PER_EPISODE:
        ...
        for i, agent in enumerate(agents):
            ...
            '''
            # 在这里插入梯度检查代码
            for name, param in agent.alg.model.named_parameters():
                if param.grad is not None:
                    print(f"Agent {i} - Gradient of {name}: min={param.grad.min().item()}, max={param.grad.max().item()}")
            ...
            print(f"Agent {i} - Total gradient norm: {total_norm}")
            '''           
