...
class ParkEnv(gym.Env):
    ...
    def _generate_map(self):
        ...
        logger.debug("边界生成完成。")
        if not self.boundary_polygon.is_valid or self.boundary_polygon.is_empty:
            logger.error("生成的边界多边形无效或为空。")
            raise ValueError("生成的边界多边形无效或为空。")
        ...
        attempts = 0
        max_attempts = 10
        while np.linalg.norm(self.start_pos - self.end_pos) < min_distance and attempts < max_attempts:
            logger.debug(f"起点和终点距离太近（{np.linalg.norm(self.start_pos - self.end_pos)}），重新选择终点。")
            self.end_pos = self._get_point_on_boundary().astype(np.float32)
            attempts += 1

        if np.linalg.norm(self.start_pos - self.end_pos) < min_distance:
    ...
    def _generate_boundary(self):
        ...
        logger.debug(f"生成 {num_points} 个随机点用于凸包计算。")
        # 计算凸包，连接最外圈的点
        try:
            hull = ConvexHull(points)
            hull_points = points[hull.vertices]
        except Exception as e:
            logger.error(f"ConvexHull 计算失败：{e}")
            raise e
    ...
    def _get_random_position(self):
        ...
        while attempts < max_attempts:
            ...
            if self.map[0, y, x] == 1:
                logger.debug(f"选择的位置：({x}, {y})")
                return np.array([x, y], dtype=np.int32)
            attempts += 1
        logger.error(f"在边界上找不到有效的位置，尝试了 {max_attempts} 次。")
        raise ValueError("在边界上找不到有效的位置。")
    ...
    def step(self, actions):
        logger.debug(f"步骤 {self.steps + 1} 开始。")
        ...
        obs = self._get_observations()
        logger.debug(f"步骤 {self.steps} 完成。")
        if self.render_mode:
            self.render()
