...
def preprocess_observations(obs_dict):
    """
    批量预处理观测字典，将每个智能体的观测图像从 (3, 128, 128) 缩放到 (3, 64, 64)，并保持 dtype 为 uint8。
    Args:
        obs_dict (dict): 包含多个智能体观测的字典，每个观测为 (3, 128, 128) 的 NumPy 数组。
    Returns:
        dict: 预处理后的观测字典，保持与原始格式相同，每个观测为 (3, 64, 64) 的 NumPy 数组。
    """
    preprocessed_obs_dict = {}

    for agent, observation in obs_dict.items():
        # 确保输入是 NumPy 数组
        if not isinstance(observation, np.ndarray):
            raise TypeError("Observation must be a NumPy array.")

        # 转换为 (H, W, C) 以适应 OpenCV
        observation = np.transpose(observation, (1, 2, 0))  # 从 (C, H, W) 到 (H, W, C)

        # 使用 OpenCV 缩放图像到 (64, 64)
        resized_observation = cv2.resize(observation, (64, 64), interpolation=cv2.INTER_AREA)

        # 转换回 (C, H, W)
        resized_observation = np.transpose(resized_observation, (2, 0, 1))  # 从 (H, W, C) 到 (C, H, W)

        # 确保数据类型为 uint8
        resized_observation = resized_observation.astype(np.uint8)

        # 将预处理后的观测存入字典
        preprocessed_obs_dict[agent] = resized_observation

    return preprocessed_obs_dict
