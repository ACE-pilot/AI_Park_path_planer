...
class ParkEnv(gym.Env):
    ...
    def _get_observations(self):
        ...
        for i, agent in enumerate(self.agents):
            ...
            ])  # 总维度：2 + (num_agents -1)*2 + 2 + 2 + 20 + (num_agents -1)*2 + 2 + 2 + 20 + 121
    ...
    def step(self, actions):
        ...
        for i, agent in enumerate(self.agents):
            ...
            if self._is_done(new_pos):
                ...
                # 仅当智能体访问过兴趣点并获得加分时，才给予终点奖励
                if self.agent_has_interest_point_bonus[i]:
                    reward += self.endpoint_bonus  # 智能体到达终点且已访问过兴趣点，给予奖励
                else:
                    pass  # 未访问过兴趣点，不给予终点奖励
    ...
    def _calculate_reward(self, old_pos, new_pos, trajectory, agent_index):
        ...
        if distance_delta > 0:
            reward += 2 * distance_delta  # 向终点移动的奖励
        else:
            reward -= 1 * abs(distance_delta)  # 远离终点的惩罚
        ...
        if displacement < 1:
            ...
            for point in interest_points:
                ...
                # 如果智能体距离兴趣点在允许范围内，且访问次数未超出限制，则给予奖励
                if min_distance <= max_reward_distance:
                    # 限制每个智能体最多访问两次，每个兴趣点最多给加分两次
                    if self.interest_points_visit_count[point_tuple][agent_name] < 2 and sum(self.interest_points_visit_count[point_tuple].values()) < 4:
                        # 计算距离奖励函数
                        proximity_reward = ((max_reward_distance - min_distance) / max_reward_distance) ** n
                        points_gained = proximity_reward * 100 #40
                        reward += points_gained

                        # 更新访问计数
                        self.interest_points_visit_count[point_tuple][agent_name] += 1
                        # 打印加分情况
