...
CONFIGS = {
    'Full (CNN-MLP)': 'seed0_full_training.csv',
    'No Slope Penalty': 'A1_nopenalty_seed0_training.csv',
    'No Elevation Obs': 'A2_noobs_seed0_training.csv',
}
...
def main():
    ...
    parser.add_argument('--output_dir', type=str,
                        default='data/results/slope_ablation')
    ...
    ax.set_title('Slope Ablation Training Curves')
    nature_style.format_episodes_axis(ax)
    ax.legend(loc='lower right')
    fig.savefig(os.path.join(args.output_dir, 'slope_ablation_training.png'))
    plt.close(fig)
    print("Saved slope_ablation_training.png")
    ...
    ax.set_title('Slope Ablation Evaluation')
    fig.savefig(os.path.join(args.output_dir, 'slope_ablation_eval_bar.png'))
    plt.close(fig)
    print("Saved slope_ablation_eval_bar.png")

    summary = pd.DataFrame({'config': names, 'mean_reward': means, 'std_reward': stds})
    summary.to_csv(os.path.join(args.output_dir, 'slope_ablation_summary.csv'), index=False)
    print("Saved slope_ablation_summary.csv")
