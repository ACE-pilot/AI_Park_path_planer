...
class ParkEnv(gym.Env):
    ...
    def _get_observations(self):
        ...
        for i, agent in enumerate(self.agents):
            ...
            # 获取所有兴趣点的位置
            interest_points = self._get_interest_points(include_tree_centers=False)
    ...
    def _calculate_reward(self, old_pos, new_pos, trajectory, agent_index):
        ...
        if abs(int(new_elevation) - int(old_elevation)) < 10:
            reward += 1  # 高差变化小，加分
        if abs(int(new_elevation) - int(old_elevation)) > 100:
            reward -= 5  # 高差变化大，减分
        ...
        distance_reward = distance_to_half_map * distance_reward_factor * 0.1
    ...
    def _is_done(self, position):
        # 检查智能体是否到达共享的终点
        if np.linalg.norm(position - self.end_pos) < 2:
            return True
        else:
            return False
